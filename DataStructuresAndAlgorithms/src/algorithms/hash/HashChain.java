...
class SortedList {
  ...
  // -------------------------------------------------------------
  public Link find(int key)         // find LinkInterview
  {
    Link current = first;          // start at first
    // until end of list,
    while (current != null && current.getKey() <= key) {                           // or key too small,
      if (current.getKey() == key)    // is this the LinkInterview?
        return current;          // found it, return LinkInterview
...
class HashChainTable {
  ...
  public void delete(int key) {
    ...
    hashArray[hashVal].delete(key); // delete LinkInterview
  }

  public Link find(int key)         // find LinkInterview
  {
    int hashVal = hashFunc(key);   // hash the key
    Link theLink = hashArray[hashVal].find(key);  // get LinkInterview
    return theLink;                // return LinkInterview
  }
}