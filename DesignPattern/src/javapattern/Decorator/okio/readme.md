...
Okio is built around two types that pack(包装) a lot of capability(能力) into a
straightforward(简单的) API:

 * [**ByteString**][3] is an immutable(不可变) sequence of bytes. For character data, `String`
   is fundamental(基本的). `ByteString` is String's long-lost brother, making it easy to
   treat binary data as a value. This class is ergonomic: it knows how to encode
   and decode itself as hex, base64, and UTF-8.

 * [**Buffer**][4] is a mutable(可变) sequence of bytes. Like `ArrayList`, you don't need
...
An elegant(简洁的) part of the `java.io` design is how streams can be layered(分层) for
transformations(转化) like encryption and compression. Okio includes its own stream
